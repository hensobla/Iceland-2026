...
**The sheet's height comes from a measurement, not from `svh`.** `svh` is the right *idea* — the small viewport, browser chrome showing, so a drawer cannot resize under the reader when iOS collapses its toolbars mid-scroll — but Chrome on iOS resolves it wrong. An 86svh drawer came up at roughly half the screen on an iPhone in portrait; Safari on the same phone was correct. It is content-independent, so it is the height being computed small rather than the panel falling back to fit-content, which is the *other* failure this section's `height` guards against (§11 and the comment on `.sheet`).

> The chase is worth recording because the first hypothesis was wrong and looked right. Both sheets share one `<dialog>` and one rule, so a difference between them has to come from content — which pointed at `height` being dropped and the dialog sizing to content, exactly the pre-fix behaviour. It is not that: at phone width **every** day's timeline already overflows the viewport, so even content-sizing yields a full-height panel. Forcing `height:auto` in a desktop browser reproduces nothing. The phrasebook is not actually fine on Chrome iOS either — it is the same wrong height, but a dense scrolling list does not read as "too short" the way a timeline with room to spare does.

`--vh` holds a hundredth of `innerHeight` and the CSS multiplies it up, behind two fallbacks (`vh`, then `svh`) so engines that never had the bug are untouched. It is set at the top of the script and **again in `openSheet`**, which is the whole trick: measuring at the moment a sheet opens gives the viewport the reader can actually see, whether iOS has its toolbars up or somebody just dragged a desktop window shorter. Setting it once at load left a resized desktop window opening a stale-height sheet. It is deliberately **not** on `resize` — iOS fires that as the toolbars slide away during a scroll, and re-measuring there would change the height of a panel being read. `orientationchange` is the exception, since that can happen with a sheet open.
...
**Everything inside `.ports` is in em**, so the bar scales as one piece. At 15.5px every value is exactly the px it used to be, so nothing moves at full size; it only matters below full size. With the icon, the gaps, the arrow margins and the padding all frozen in px, **82 of the widest row's 337px could not shrink at all**, and holding that row to one line would have meant taking the type to about 8px on a small phone. Scaled, the same row needs about 11px. The 16px `width`/`height` attributes on the inline ship SVG are overridden in CSS to `1.032em` for the same reason.
...
> **Focus goes back to whatever raised the sheet, without a ring when a pointer closed it.** Safari counts a programmatic `focus()` as `:focus-visible`, so closing an itinerary by tap left a ring sitting on the Details row until something else was touched. Suppressing it outright would take it from keyboard users, who are the ones who need to see where focus went after Escape, so the input modality is tracked instead: `html[data-nofv]` goes on at a pointer-driven close and comes off at the next keystroke. `keydown` is captured so it lands *before* the browser moves focus, which is what lets the first Tab out of a suppressed element still draw its ring, and the attribute is lifted again on the restored element's `blur` so a touch-only reader does not carry a page-wide ring suppression for the rest of the session.

> **It covers every exit, because there is only one.** The close button, the backdrop and Escape all call `closeSheet`, and both the phrasebook and the itinerary go through the same function; only `back` differs. The override is `html[data-nofv] :focus-visible` at specificity (0,2,1) against every ring rule on the page at (0,2,0), so it wins on all of them regardless of source order. The matrix checked is sheet type x exit: day and phrasebook, each closed by X, by backdrop and by Escape.

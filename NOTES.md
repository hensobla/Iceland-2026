...
**Sagas & Songs (14 August).** Windstar runs a complimentary Destination Discovery Event, *Sagas & Songs at the Blue Church*, during the Seyðisfjörður overnight: a local duo singing and telling Icelandic sagas, about 45 minutes, with drinks and canapés and a ring dance outside afterwards. It is a fixture of the Around Iceland itinerary, not a maybe, and it runs late afternoon or early evening — after the 5:35 pm return from the land day, which is why it sits on the **14th** and not on the free morning of the 15th, where a vague version of it used to live.

What is *not* publicly checkable is the hour. Windstar sets it on board and puts it in the daily programme; the per-sailing time is on no public page, and `windstarcruises.com` returns 403 to anything that is not a browser, so it cannot be scraped either. The note therefore says what the event is, that it is included, and that Jeffrey can confirm the time with the ship — which is the one thing on the page a traveller genuinely cannot resolve alone.
